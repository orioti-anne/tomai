...
def save_best_growth_results(model, features, score, mae):
    ...
    top_importances.sort_values(ascending=True).plot(kind='barh', color='navy')

    plt.title(f'최종 생육 모델 피처별 중요도\nR2: {score:.4f} | MAE: {mae:.6f}')
    ...
    info_text = "\n".join(
        ...
        [f"{idx}. {name}: {value:.4f}" for idx, (name, value) in enumerate(top_importances.items(), start=1)]
    )
...
def run_optimized_diet_prediction():
    ...
    with app.app_context():
        ...
        df_grow['RGR_HEIGHT'] = (np.log(df_grow['PLANT_HEIGHT']) - np.log(df_grow['PREV_HEIGHT'])) / df_grow['DAYS_DIFF']
        ...
        save_best_growth_results(model, features, score, mae)
        plt.show()
