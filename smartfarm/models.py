...
class ProdSummary(db.Model):
    ...
    cult_total_quantity = db.Column('CULT_TOTAL_QUANTITY', db.Float)
    cult_total_sales = db.Column('CULT_TOTAL_SALES', db.Float)
    yield_per_area = db.Column('YIELD_PER_AREA', db.Float)
    unit_price = db.Column('UNIT_PRICE', db.Float)
    origin_type = db.Column('ORIGIN_TYPE', db.Integer, default=0)
...
class EnvCleaned(db.Model):
    ...
    cult_id = db.Column('CULT_ID', db.Integer, db.ForeignKey('cultivations.cult_id'), nullable=False)
    measure_time = db.Column('MEASURE_TIME', db.DateTime, nullable=False)
    out_temp = db.Column('OUT_TEMP', db.Float)
    out_wind_direction = db.Column('OUT_WIND_DIRECTION', db.Float)
    out_wind_speed = db.Column('OUT_WIND_SPEED', db.Float)
    out_solar_rad = db.Column('OUT_SOLAR_RAD', db.Float)
    out_acc_solar_rad = db.Column('OUT_ACC_SOLAR_RAD', db.Float)
    rain_detection = db.Column('RAIN_DETECTION', db.Integer)
    in_temp = db.Column('IN_TEMP', db.Float)
    in_humidity = db.Column('IN_HUMIDITY', db.Float)
    in_co2 = db.Column('IN_CO2', db.Float)
    soil_temp = db.Column('SOIL_TEMP', db.Float)
    out_acc_solar_rad_status = db.Column('OUT_ACC_SOLAR_RAD_STATUS', db.Integer, default=0)
    in_temp_status = db.Column('IN_TEMP_STATUS', db.Integer, default=0)
    in_humidity_status = db.Column('IN_HUMIDITY_STATUS', db.Integer, default=0)
    in_co2_status = db.Column('IN_CO2_STATUS', db.Integer, default=0)
    measure_date = db.Column('MEASURE_DATE', db.Date)
    measure_hour = db.Column('MEASURE_HOUR', db.Integer)
    created_at = db.Column('CREATED_AT', db.DateTime, default=datetime.now)
...
class EnvSummary(db.Model):
    ...
    daily_out_temp = db.Column('DAILY_OUT_TEMP', db.Float)
    daily_acc_solar = db.Column('DAILY_ACC_SOLAR', db.Float)
    daily_rain_detection = db.Column('DAILY_RAIN_DETECTION', db.Integer)
    daily_in_temp = db.Column('DAILY_IN_TEMP', db.Float)
    daily_in_humidity = db.Column('DAILY_IN_HUMIDITY', db.Float)
    daily_in_co2 = db.Column('DAILY_IN_CO2', db.Float)
    daily_soil_temp = db.Column('DAILY_SOIL_TEMP', db.Float)
    acc_temp = db.Column('ACC_TEMP', db.Float)
    acc_solar = db.Column('ACC_SOLAR', db.Float)
    created_at = db.Column('CREATED_AT', db.DateTime, default=datetime.now)
...
class GrowSummary(db.Model):
    ...
    cult_id = db.Column('CULT_ID', db.Integer, db.ForeignKey('cultivations.cult_id'), nullable=False)
    inspect_date = db.Column('INSPECT_DATE', db.Date, nullable=False)
    growth_days = db.Column('GROWTH_DAYS', db.Integer)
    plant_num = db.Column('PLANT_NUM', db.Integer)
    branch_num = db.Column('BRANCH_NUM', db.Integer)
    plant_height = db.Column('PLANT_HEIGHT', db.Float)
    growth_length = db.Column('GROWTH_LENGTH', db.Float)
    leaf_count = db.Column('LEAF_COUNT', db.Integer)
    leaf_length = db.Column('LEAF_LENGTH', db.Float)
    leaf_width = db.Column('LEAF_WIDTH', db.Float)
    branch_width = db.Column('BRANCH_WIDTH', db.Float)
    cluster_height = db.Column('CLUSTER_HEIGHT', db.Float)
    cluster_num = db.Column('CLUSTER_NUM', db.Integer)
    flowers_per_cluster = db.Column('FLOWERS_PER_CLUSTER', db.Float)
    blooming_per_cluster = db.Column('BLOOMING_PER_CLUSTER', db.Float)
    fruits_per_cluster = db.Column('FRUITS_PER_CLUSTER', db.Float)
    created_at = db.Column('CREATED_AT', db.DateTime, default=datetime.now)
    origin_type = db.Column('ORIGIN_TYPE', db.Integer, default=0)
