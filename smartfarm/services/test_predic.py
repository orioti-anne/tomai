...
def run_default_prediction(**kwargs) -> Dict[str, Any]:
    ...
    harvest_target_date = datetime.combine(p_date + timedelta(days=105), datetime.min.time())
    ...
    if model_p:
        try:
            df = _get_recent_market_data()
            if not df.empty:
                price_s = df["PRICE_PER_KG"]
                temp_s = df["AVG_TEMP"]
                curr_price = float(price_s.iloc[-1])

                input_data = pd.DataFrame([{
                    "MA_30D": price_s.tail(30).mean(),
                    "MA_90D": price_s.tail(90).mean(),
                    "YOY_RATIO": curr_price / price_s.iloc[-365] if len(price_s) >= 365 else 1.0,
                    "GDD_30D": (temp_s.tail(30) - 10).clip(lower=0).sum(),
                    "TEMP_MA_30D": temp_s.tail(30).mean(),
                    "WEEK_SIN": np.sin(2 * np.pi * harvest_target_date.isocalendar()[1] / 52),
                    "WEEK_COS": np.cos(2 * np.pi * harvest_target_date.isocalendar()[1] / 52),
                    "MONTH": harvest_target_date.month
                }])

                pred_val = model_p.predict(input_data)[0]
                final_price = float(np.expm1(pred_val))
        except Exception as e:
            print(f"[PRICE PREDICT ERROR] {e}")
    if final_price is None or final_price <= 100:
        final_price = _get_latest_market_price()
    ...
    # 2) 생산량 예측
    # ---------------------------------------------------------
    model_y, path_y = _load_model([
        "yield_prediction_model.joblib",
    ])

    final_yield_total = 0.0
    yield_fallback_message = None
    yield_fallback_reason = None

    if model_y and cult_id:
        try:
            X_y, feature_error = _build_yield_feature_row(int(cult_id))

            if X_y is not None and not X_y.empty:
                print(f"[YIELD MODEL PATH] {path_y}")
                pred_y = model_y.predict(X_y)[0]
                final_yield_total = float(np.expm1(pred_y))
                final_yield_total = max(0.0, final_yield_total)
                print(f"[YIELD PREDICT OK] final_yield_total={final_yield_total}")
            else:
                yield_fallback_message = feature_error or "생산량 예측 입력정보가 부족하여 임시 기준값으로 계산되었습니다."
                yield_fallback_reason = "feature_unavailable"

        except Exception as e:
            print(f"[YIELD PREDICT ERROR] {type(e).__name__}: {e}")
            yield_fallback_message = "생산량 예측 중 오류가 발생하여 임시 기준값으로 계산되었습니다."
            yield_fallback_reason = "predict_error"

    if final_yield_total <= 0:
        if not yield_fallback_message:
            yield_fallback_message = "생산량 예측 모델을 실행할 수 없어 임시 기준값으로 계산되었습니다."
        if not yield_fallback_reason:
            yield_fallback_reason = "model_unavailable"
        ...
        return _build_fallback_result(
            ...
            message=yield_fallback_message,
            reason_code=yield_fallback_reason,
        )
    ...
    result = {
        ...
        "prediction_source": "model_v4" if path_p and "v4" in path_p.lower() else "model",
        "prediction_confidence": "medium",
        "prediction_message": "V4 고도화 시세 모델이 적용되었습니다." if path_p and "v4" in path_p.lower() else "재배 및 환경데이터를 기반으로 계산되었습니다.",
        "yield_fallback_reason": None,
    }
